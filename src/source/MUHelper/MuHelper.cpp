...
int CMuHelper::ActivatePet()
{
    ...
    {
        SocketClient->ToGameServer()->SendPetCommandRequest(PetType::DarkRaven, PetCommandMode::AttackWithOwner,
                                                            0xFFFF);
    }
...
int CMuHelper::Attack()
{
    if (m_iCurrentTarget == -1)
    {
        ...
        {
            m_iComboState = 0;
            return m_config.bRandomMoveWhenIdle ? MoveRandomlyWhenIdle() : 0;
        }
    }
...
ActionSkillType CMuHelper::SelectAttackSkill()
{
    const size_t safeSize =
        std::min({m_config.aiSkill.size(), m_config.aiSkillCondition.size(), m_config.aiSkillInterval.size()});
    for (int i = 1; i < (int)safeSize; i++)
    {
        ...
        if ((m_config.aiSkillCondition[i] & ON_TIMER) && m_config.aiSkillInterval[i] != 0 && m_iSecondsElapsed > 0 &&
...
int CMuHelper::Regroup()
{
    ...
    {
        if (SimulateMove(m_posOriginal) != MoveResult::Reached)
        {
            return 0;
        }
...
CMuHelper::MoveResult CMuHelper::SimulateMove(POINT posMove)
{
    Hero->MovementType = MOVEMENT_MOVE;
    TargetX = static_cast<int>(posMove.x);
    TargetY = static_cast<int>(posMove.y);
...
int CMuHelper::MoveRandomlyWhenIdle()
{
    if (Hero->Movement)
    {
        return 0;
...
bool CMuHelper::ShouldObtainItem(int iItemId)
{
    ...
    if (m_config.bPickExtraItems)
    {
        ...
        {
            if (strDisplayName.find(str) != std::wstring::npos || std::wcsstr(itemName.data(), str.c_str()) != nullptr)
            {
                return true;
            }
        }
